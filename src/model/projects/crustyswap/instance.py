...
from eth_account import Account
from src.model.projects.crustyswap.constants import (
    CONTRACT_ADDRESSES, 
...
class CrustySwap:
    def __init__(
        ...
        self.eth_web3 = None
        self.galileo_contract = self.galileo_web3.web3.eth.contract(address=DESTINATION_CONTRACT_ADDRESS, abi=CRUSTY_SWAP_ABI)
    ...
    async def get_galileo_balance(self) -> float:
        """Get native GALILEO balance."""
        try:
            balance_wei = await self.galileo_web3.web3.eth.get_balance(self.wallet.address)
            return float(self.galileo_web3.web3.from_wei(balance_wei, 'ether'))
        except Exception as e:
            logger.error(f"[{self.account_index}] Failed to get GALILEO balance: {str(e)}")
            return 0
    ...
    async def wait_for_balance_increase(self, initial_balance: float) -> bool:
        """Wait for GALILEO balance to increase after refuel."""
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            current_balance = await self.get_galileo_balance()
            if current_balance > initial_balance:
                logger.success(
                    f"[{self.account_index}] Balance increased from {initial_balance:.9f} to {current_balance:.9f} GALILEO"
                )
                return True
    ...
    async def refuel(self) -> bool:
        """Refuel GALILEO from one of the supported networks."""
        try:
            await self.initialize()
            # Get current GALILEO balance before refuel
            initial_balance = await self.get_galileo_balance()
            logger.info(f"[{self.account_index}] Initial GALILEO balance: {initial_balance:.9f}")
            ...
            has_enough_galileo = await self.check_available_galileo(amount_wei, contract)
            if not has_enough_galileo:
                logger.error(f"[{self.account_index}] Not enough GALILEO in the contract for your amount of ETH deposit, try again later")
                return False
    ...
    async def check_available_galileo(self, eth_amount_wei, contract, max_retries=5, retry_delay=5) -> bool:
        """
        Check if there is enough GALILEO in the Crusty Swap contract to fill a buy order.
        ...
        Returns:
            bool: True if there is enough GALILEO, False otherwise
        """
        for attempt in range(1, max_retries + 1):
            try:
                # Get available GALILEO in the contract
                available_galileo_wei = await self.galileo_web3.web3.eth.get_balance(DESTINATION_CONTRACT_ADDRESS)
                ...
                # Get GALILEO price from contract (in USD with 8 decimals)
                galileo_price_usd = await contract.functions.getChainPricePerETH(DESTINATION_CHAIN_ID).call()
                
                # Calculate how many GALILEO we should receive for our ETH
                ...
                # Calculate GALILEO amount from USD value
                galileo_price_usd_real = galileo_price_usd / 10**8  # Convert to real USD value
                expected_galileo_amount = eth_value_usd / galileo_price_usd_real
                expected_galileo_amount_wei = int(expected_galileo_amount * 10**18)
                ...
                logger.info(f"[{self.account_index}] GALILEO price: ${galileo_price_usd_real:.4f}")
                logger.info(f"[{self.account_index}] Available GALILEO: {self.galileo_web3.web3.from_wei(available_galileo_wei, 'ether')} GALILEO")
                logger.info(f"[{self.account_index}] Expected to receive: {expected_galileo_amount:.4f} GALILEO")
                
                # Check if there's enough GALILEO in the contract
                has_enough_galileo = available_galileo_wei >= expected_galileo_amount_wei
                
                if has_enough_galileo:
                    logger.success(f"[{self.account_index}] Contract has enough GALILEO to fill the order")
                else:
                    logger.warning(f"[{self.account_index}] Contract doesn't have enough GALILEO! " 
                                f"Available: {self.galileo_web3.web3.from_wei(available_galileo_wei, 'ether')} GALILEO, "
                                f"Needed: {expected_galileo_amount:.4f} GALILEO")
                    
                return has_enough_galileo
                
            except Exception as e:
                if attempt < max_retries:
                    logger.warning(f"[{self.account_index}] Attempt {attempt}/{max_retries} failed to check available GALILEO: {str(e)}")
                    logger.info(f"[{self.account_index}] Retrying in {retry_delay} seconds...")
                    await asyncio.sleep(retry_delay)
                else:
                    logger.error(f"[{self.account_index}] All {max_retries} attempts failed to check available GALILEO: {str(e)}")
                    return False
        
        # We should never reach here, but just in case
        return False

    async def _get_galileo_balance(self, address) -> float:
        """Get native GALILEO  balance for a specific address."""
        try:
            balance_wei = await self.galileo_web3.web3.eth.get_balance(address)
            return float(self.galileo_web3.web3.from_wei(balance_wei, 'ether'))
        except Exception as e:
            logger.error(f"[{self.account_index}] Failed to get GALILEO balance: {str(e)}")
            return None
    ...
    async def _wait_for_balance_increase(self, initial_balance: float, address: str) -> bool:
        ...
        # Check balance every 5 seconds until timeout
        while asyncio.get_event_loop().time() - start_time < timeout:
            current_balance = await self._get_galileo_balance(address)
            if current_balance > initial_balance:
                logger.success(
                    f"[{self.account_index}] Balance increased from {initial_balance} to {current_balance} GALILEO"
                )
                return True
    ...
    async def send_refuel_from_one_to_all(self, address) -> bool:
        ...
        try:
            initial_balance = await self._get_galileo_balance(address)
            if initial_balance is None:
                logger.error(f"[{self.account_index}] Failed to get GALILEO balance for address: {address}")
                return False
            logger.info(f"[{self.account_index}] Initial GALILEO balance: {initial_balance}")
            if initial_balance > self.config.CRUSTY_SWAP.MINIMUM_BALANCE_TO_REFUEL:
    ...
    async def refuel_from_one_to_all(self, private_keys_to_distribute) -> bool:
        """Refuel GALILEO from one of the supported networks to multiple addresses."""
        try:
            await self.initialize()
